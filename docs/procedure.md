...
-	Measure **VDDA** and **VDDD** on the SCC and set them to **1.2V** in the according entries in the chip configuration file.
	- YARR: change ``SldoAnalogTrim`` and ``SldoDigitalTrim``. If you still get ``data not valid`` errors, adjust VDDA to a value that this error disappears. More information in [FAQ & Troubleshooting](../troubleshooting).
	- BDAQ: change ``VREF_A_TRIM`` and ``VREF_D_TRIM``.
...
-	When running threshold scan, make sure the range of injected charge covers the expected threshold. The conversion is approximately 1 DAC = 10 electrons.
	- YARR: ``InjVcalDiff``
